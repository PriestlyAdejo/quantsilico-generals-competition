"""STAGE5_TEACHER_R2 STEP2+STEP3: self-play label extraction + BC distillation.

STEP2: replays each DECISIVE STEP1 self-play transcript through the pinned
engine and pairs BOTH seats' canonical legal observations (observe_one_jax
with per-seat tracked fog memory - same legal path as BC-A, EV-0050/0060)
with the action the teacher executed that tick in that seat (engine-verified
by construction; the teacher is the identical HunterAgent policy in both
seats). Labels whose action index falls outside legal_mask_one_jax are
ENGINE_SILENT_PASS and are excluded (predeclared). Winner + loser seats both
kept (predeclared). Draw-at-truncation games contribute no labels.

STEP3: trains the BC-A canonical small CNN (same architecture/hyperparameters
as bc_a_train_pilot.py) on the teacher data with the R1-identical
deterministic game-disjoint split assignment (sorted decisive seeds:
[:14] train / [14:17] holdout_games / [17:20] holdout_maps). Screening gate
is IDENTICAL to BC-A-FULL (EV-0060): held-out top-1 strictly above BOTH
legal-uniform and majority-pass on >= 1 non-empty holdout split.
...
OUT_ROOT = REPO / "experiments/marathon/teacher_r2/step3_bc"
MAX_HW = 21
...
def build_features(docs: list[dict], split_of: dict[int, str]) -> dict[str, dict]:
    ...
    for doc in docs:
        ...
        memory0 = empty_memory()
        memory1 = empty_memory()
        for a0, a1 in zip(doc["actions_hunter"], doc["actions_opponent"]):
            for seat, action, mem in ((0, a0, memory0), (1, a1, memory1)):
                spatial, global_vec, new_mem = observe_one_jax(state, seat, mem)
                mask = legal_mask_one_jax(state, seat)
                idx = engine_action_to_index(action)
                if seat == 0:
                    memory0 = new_mem
                else:
                    memory1 = new_mem
...
def main() -> int:
    ...
    print(f"decisive games: {len(decisive)}/20; splits train={len(seeds[:14])} "
    ...
    results = {
        "kind": "STAGE5_TEACHER_R2_BC_RESULT",
        ...
        "step1_games": len(docs),
        "decisive_games": len(decisive),
        "decisive_seeds": seeds,
    ...
    result_path = OUT_ROOT / "teacher_r2_result.json"
