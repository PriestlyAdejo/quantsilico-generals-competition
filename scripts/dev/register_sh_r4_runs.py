"""Register SH-R4 run/checkpoint records from local evidence (EV-0032)."""
...
R3_ROOT = REPO / "experiments/marathon/screening_round_4_runs"
EXPERIMENT_ID = "experiment#sh-r4-budget-escalation#f4a79a0412e9"
POD_ID = "tinlsf21h08qd2"
SOURCE_COMMIT = "7daa33f"
...
def main() -> int:
    ...
    for arm_dir in sorted(R3_ROOT.iterdir()):
        ...
        run_record = {
            ...
            "COMMAND": (
                "remote_sh_r4_orchestrator.sh -> run_sh_r1_arm.py --arm-id "
                f"{name} --budget-transitions 1966080 --seed {summary['seed']} (pool-fixed runner)"
            ),
            ...
            "RESULT": (
                ...
                f"STEADY_E2E_TPS_{summary['throughput']['end_to_end_tps']:.0f} EV-0032"
            ),
        }
