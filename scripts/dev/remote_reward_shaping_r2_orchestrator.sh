#!/usr/bin/env bash
# REWARD-SHAPING-R2 round orchestrator (run ON the pod via remote_orchestrator_with_stop.sh).
...
ROUND_LOG=/workspace/reward_shaping_r2_round.log
OUT_ROOT=/workspace/screening_runs_reward_shaping_r2
...
mkdir -p "$OUT_ROOT"
echo "=== REWARD-SHAPING-R2 round start $(stamp) commit $(cd $REPO && git rev-parse --short HEAD)" >> "$ROUND_LOG"
echo "=== REWARD-SHAPING-R2 arm table (predeclared): CONTROL(none)/LAND-POTENTIAL(1.0) x seeds {20260825 20260827}, 240 updates @ 256x32" >> "$ROUND_LOG"

for spec in \
  "RSH2-A0-CONTROL-S1 none 0.0 20260825" \
  "RSH2-A0-CONTROL-S2 none 0.0 20260827" \
  "RSH2-A1-LAND-S1 land_potential 1.0 20260825" \
  "RSH2-A1-LAND-S2 land_potential 1.0 20260827"
do
  set -- $spec
...
echo "=== REWARD-SHAPING-R2 round end $(stamp)" >> "$ROUND_LOG"
