"""Pre-game serving sanity probe for the STAGE6_DISTILL_PPO_R1 gameplay arbiter.

EV-0034 precedent + scale/rwb1/t2 probe pattern. Two layers, local/CPU,
deterministic:

L1 PARAMETER EFFECT (canonical 8-plane path): each loaded D1 terminal
...
Requires the D1 terminal checkpoints fetched to
experiments/marathon/distill_ppo_r1/d1/<ARM>/raw.npz. Must PASS before any
gameplay games are counted. Exit 0 = PASS, 2 = DEFECT.
"""
...
CANDIDATES = {
    "distill_ppo_s1": REPO / "baselines/distill_ppo_s1/main.py",
    "distill_ppo_s2": REPO / "baselines/distill_ppo_s2/main.py",
}
CHECKPOINTS = {
    "distill_ppo_s1": REPO
    / "experiments/marathon/distill_ppo_r1/d1/DISTILL-PPO-S1/raw.npz",
    "distill_ppo_s2": REPO
    / "experiments/marathon/distill_ppo_r1/d1/DISTILL-PPO-S2/raw.npz",
}
...
def main() -> int:
    ...
    distinct = float(np.max(np.abs(loaded["distill_ppo_s1"] - loaded["distill_ppo_s2"])))
