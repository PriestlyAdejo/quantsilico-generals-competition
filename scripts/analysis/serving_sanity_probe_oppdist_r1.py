"""Pre-game serving sanity probe for the STAGE6_OPPDIST_R1 gameplay arbiter.

EV-0034 precedent + scale/rwb1/t2/distill probe pattern. Two layers,
local/CPU, deterministic:

L1 PARAMETER EFFECT (canonical 8-plane path): each loaded OPPDIST terminal
   checkpoint must change inference outputs vs freshly-initialised
   parameters AND vs the MARATHON_BASELINE_V0 warm-start (proves training
   changed the weights), and the two checkpoints must produce distinct
...
experiments/marathon/screening_runs/STAGE6-OPPDIST-R1/<ARM>/raw.npz. Must
PASS before any gameplay games are counted. Exit 0 = PASS, 2 = DEFECT.
"""
...
CANDIDATES = {
    "oppdist_s1": REPO / "baselines/oppdist_s1/main.py",
    "oppdist_s2": REPO / "baselines/oppdist_s2/main.py",
}
CHECKPOINTS = {
    "oppdist_s1": REPO
    / "experiments/marathon/screening_runs/STAGE6-OPPDIST-R1/OPPDIST-B0-TEACHOPP-S1/raw.npz",
    "oppdist_s2": REPO
    / "experiments/marathon/screening_runs/STAGE6-OPPDIST-R1/OPPDIST-B0-TEACHOPP-S2/raw.npz",
}
BASELINE_NPZ = (
    Path.home()
...
def main() -> int:
    ...
    baseline = logits_for(load_tree(BASELINE_NPZ, template))
    loaded = {}
    for name, npz in CHECKPOINTS.items():
        ...
        vs_baseline = float(np.max(np.abs(loaded[name] - baseline)))
        report["l1"][name] = {
            "max_abs_diff_vs_fresh": differs,
            "max_abs_diff_vs_baseline_warm_start": vs_baseline,
            "weights_load_and_matter": differs > 1e-6 and vs_baseline > 1e-6,
        }
        ok = ok and differs > 1e-6 and vs_baseline > 1e-6
    distinct = float(np.max(np.abs(loaded["oppdist_s1"] - loaded["oppdist_s2"])))
