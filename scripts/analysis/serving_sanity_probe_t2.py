"""Pre-game serving sanity probe for the STAGE5 T2 K1 gameplay arbiter.

EV-0034 precedent + rwb1 probe pattern. Two layers, local/CPU, deterministic:

L1 PARAMETER EFFECT (16-plane k1 path): each loaded terminal checkpoint must
   change inference outputs vs freshly-initialised 16-plane parameters, and
   the two checkpoints must produce distinct outputs on identical inputs.
...
from generals_bot.competition_native_jax.obs_memory import N_GLOBAL  # noqa: E402
...
K1_PLANES = 16
CANDIDATES = {
    "s5_t2_k1_s1": REPO / "baselines/s5_t2_k1_s1/main.py",
    "s5_t2_k1_s2": REPO / "baselines/s5_t2_k1_s2/main.py",
}
CHECKPOINTS = {
    "s5_t2_k1_s1": REPO
    / "experiments/marathon/screening_runs/STAGE5-T2-R1/artefacts/S5-T2-K1-S1/raw.npz",
    "s5_t2_k1_s2": REPO
    / "experiments/marathon/screening_runs/STAGE5-T2-R1/artefacts/S5-T2-K1-S2/raw.npz",
}
...
def logits_for(params: dict) -> np.ndarray:
    spatial = np.zeros((K1_PLANES, MAX_HW, MAX_HW), dtype=np.float32)
    spatial[0, 10, 10] = 1.0
    spatial[1, 10, 11] = 1.0
...
def main() -> int:
    ...
    template = init_params(jax.random.PRNGKey(0), spatial_planes=K1_PLANES)
    ...
    distinct = float(np.max(np.abs(loaded["s5_t2_k1_s1"] - loaded["s5_t2_k1_s2"])))
