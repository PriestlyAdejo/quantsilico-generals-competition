...
_ARCH = {
    ...
    # Alias: principal pure-PyTorch challenger (no PyG dependency).
    "recurrent_graph_belief_v2_pure_torch": (RecurrentGraphBeliefPolicy, GraphConfig),
}
...
def build_model(architecture: str, config: dict[str, Any] | None = None) -> nn.Module:
    ...
    raw = dict(config or {})
    # Canonical architecture id stored on config (strip pure_torch alias).
    canonical = architecture.removesuffix("_pure_torch")
    raw["architecture"] = canonical
    if canonical.endswith("_v1"):
        raw.setdefault("action_head", "flat_linear_v1")
        raw.setdefault("schema_version", 1)
    if canonical.endswith("_v2"):
