"""Shared serving for STAGE6_DISTILL_PPO_R1 D1 checkpoints (EVAL_ONLY).

Serves a registered DISTILL-PPO terminal checkpoint (raw weights by default,
EMA via DISTILL_WEIGHTS=ema) through the official stdio protocol using the
canonical 8-plane JaxTransformerPolicy (parity-proven inference lineage; NO
temporal history planes in this round per plan).
...
CHECKPOINT_ROOTS = [
    _REPO / "experiments/marathon/distill_ppo_r1/d1",
]


class _DistillPolicy:
...
def serve(arm_id: str) -> None:
    override = os.environ.get("DISTILL_CKPT_DIR")
    npz = None
    if override:
        npz = Path(override) / f"{os.environ.get('DISTILL_WEIGHTS', 'raw')}.npz"
    else:
        weights = os.environ.get("DISTILL_WEIGHTS", "raw")
    ...
    if npz is None or not npz.is_file():
        raise SystemExit(f"DISTILL checkpoint not found for arm {arm_id}")
    template = init_params(jax.random.PRNGKey(0))
    params = load_jax_checkpoint(npz, template)
    run_agent(_DistillPolicy(JaxTransformerPolicy(params), f"distill_{arm_id.lower()}"),
              deterministic=True)
