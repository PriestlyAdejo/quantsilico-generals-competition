"""Shared serving for STAGE6_OPPDIST_R1 terminal checkpoints (EVAL_ONLY).

Serves a registered OPPDIST arm terminal checkpoint (raw weights by default,
EMA via OPPDIST_WEIGHTS=ema) through the official stdio protocol using the
canonical 8-plane JaxTransformerPolicy (parity-proven inference lineage; the
frozen-teacher opponent is a training-rollout property only - serving is the
canonical path). PPO_SEMANTICS: EVAL_ONLY - never touches training action
selection.
"""
...
CHECKPOINT_ROOTS = [
    _REPO / "experiments/marathon/screening_runs/STAGE6-OPPDIST-R1",
]


class _OppdistPolicy:
...
def serve(arm_id: str) -> None:
    override = os.environ.get("OPPDIST_CKPT_DIR")
    npz = None
    if override:
        npz = Path(override) / f"{os.environ.get('OPPDIST_WEIGHTS', 'raw')}.npz"
    else:
        weights = os.environ.get("OPPDIST_WEIGHTS", "raw")
    ...
    if npz is None or not npz.is_file():
        raise SystemExit(f"OPPDIST checkpoint not found for arm {arm_id}")
    template = init_params(jax.random.PRNGKey(0))
    params = load_jax_checkpoint(npz, template)
    run_agent(_OppdistPolicy(JaxTransformerPolicy(params), f"oppdist_{arm_id.lower()}"),
              deterministic=True)
