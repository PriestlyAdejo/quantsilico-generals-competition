...
### `EV-0038` — Curriculum family R1 CLOSED as clean negative: both advancing variants ALL-DRAW vs control and baseline (96/96 games OK, zero faults), NO_PROMOTION; win-conversion confirmed as the binding programme constraint

- Source class: `LOCAL_RUNTIME_EVIDENCE` (curriculum_r1_gameplay_eval_plan.yaml, predeclared rules applied unchanged)
- Results (EV-0017 evaluator, seat-swapped, competition mode, run dir paired_eval_runs/curr1_advancing_variants): CURR1-A2-FAR-21-S1 (24 pairs / 48 games vs A0-CONTROL-S1 + marathon_baseline_v0): ALL DRAW at truncation, CS lower -0.1212 -> NO_PROMOTION. CURR1-A1-CLOSE-8-S1 (24 pairs / 48 games, same opponents): identical pattern, CS lower -0.1212 -> NO_PROMOTION. Attribution OK 96/96, candidate faults 0.
- Scientific accumulation (recorded honestly): three independent 2M-transition PPO lines - horizon finalists (EV-0035), curriculum control and both variants (this entry) - ALL land at exact draw parity with the heuristic baseline and each other. Spawn distance (8/17/21) does not convert into gameplay strength at this budget/scale; the +0.06%/+0.87% screening margins were noise as flagged (EV-0036). The binding constraint is WIN CONVERSION within the 1200-turn truncation, robust across geometry, horizon, and curriculum interventions. Candidate programme-level levers for later families: reward shaping for progress/kill pressure (reward-integrity family), larger budgets/capacity (Stage 5), deathtouch/draw-regime interaction analysis. No rule relaxation; curriculum R1 family question ANSWERED and closed.
- Registered: 2 candidate + 2 evaluation records (register_curr1_gameplay_eval.py). NEXT_SAFE_ACTION per programme: raw-vs-EMA evaluation family (cheapest canonical next: EMA maintained in every arm; CPU-adjudicable).

### `EV-0039` — RAW-VS-EMA-R1 adjudicated UNRESOLVED: EMA and raw from the same run are gameplay-indistinguishable (48/48 draws, zero faults); EMA remains serving default per predeclared rule

- Source class: `LOCAL_RUNTIME_EVIDENCE` (raw_vs_ema_r1_plan.yaml, predeclared rules applied unchanged)
- Setup: EMA terminal weights vs raw terminal weights from the SAME CURR1-A0-CONTROL-S1 run (2M transitions, competition-default boards); 12 seat-swapped pairs each vs curr1-a0-control-s1 (raw) and marathon_baseline_v0; EV-0017 evaluator; CPU-only; wall 1969s.
- Result: ALL 48 games DRAW at 1200-turn truncation; attribution OK 48/48; candidate faults 0; CS lower -0.1212 -> promotion pathway NO_PROMOTION.
- Predeclared decision applied: draws-only = UNRESOLVED between the lineages; EMA REMAINS the serving default (status quo preserved, no behaviour change). The family question receives a genuine non-separation at this budget; EXECUTION_PLAN 9.3 stands - raw vs EMA is not the binding lever. Combined with EV-0035/EV-0038: four independent 2M-scale PPO configurations now all land at identical draw parity; WIN CONVERSION remains the programme-level constraint.
- Registered: candidate#curr1-a0-control-s1-ema#714d74592ef1, evaluation#curr1-a0-control-s1-ema-raw-vs-ema-r1#3bb10cc39b4d.
- Operator correction recorded: AGENT_LOOP_TICK_MARATHON and AGENT_LOOP_TICK_MAREXEC sentinel text loops are DEPRECATED FAILED mechanisms (not Qoder /loop). Continuation moves to the official Qoder CLI /loop created by the operator plus the project command /marathon-exec (.qoder/commands/marathon-exec.md).

### `EV-0040` — Project command /marathon-exec created (state-discovering executor); TOPADV-R1 knob implemented + tested + smoke-passed; round PREDECLARED and REGISTERED

- Source class: `HARNESS_EVIDENCE` + `LOCAL_RUNTIME_EVIDENCE`
- Continuation mechanism correction (operator): AGENT_LOOP_TICK_MARATHON and AGENT_LOOP_TICK_MAREXEC sentinel text loops are DEPRECATED FAILED mechanisms (EV-0036 class; they cannot drive agent turns). The operator will register the real Qoder CLI /loop; project-level custom prompt command created: .qoder/commands/marathon-exec.md (name/description frontmatter per project convention verified against the qodercli bundle's project-commands loader). It discovers live state (lease -> RunPod-first -> ACTIVE_STATE/registry/ledger -> execute NEXT_SAFE_ACTION), never hard-codes the current family, and retires only at canonical completion or all-scope hard blocker.
- TOP_ADVANTAGE_FRACTIONS family launched as the next canonical Stage-4A lever for the binding win-conversion constraint (EV-0038/EV-0039 accumulation). Knob: train/competition_native_jax/top_advantage_jax.py top_advantage_mask() - keeps PG signal on top ceil(f*n) |advantage| transitions, exact values preserved, ties at threshold included, 1.0 = exact identity. PPO_SEMANTICS UNCHANGED (training-objective ablation; serving untouched; control arms byte-identical path).
- Validation: 6 unit tests PASS (identity, half-fraction exactness, k>=1, tie inclusion, no rescaling, invalid-range rejection); ruff clean; CPU smoke (TOPADV-SMOKE-F05, 2 updates, fraction 0.5): ratio 1.0, all finite, valid_share 1.0, stop BUDGET_REACHED (only NO_VLOSS_REDUCTION elimination, expected at 2 updates).
- TOPADV-R1 PREDECLARED (experiments/marathon/topadv_round_1_plan.yaml: CONTROL/TOP-50/TOP-25 x seeds {20260821,20260823}, 240 updates @ 256x32, matched CURR1 geometry/budget; advancement rules mirror CURR1 with telemetry-grade-only sub-1% margins; gameplay remains arbiter) and REGISTERED pre-launch: experiment#top-advantage-fractions-r1#5166e1a5d9db. Compute: RunPod A40 via remote self-stop wrapper (EV-0036 billing invariant).
